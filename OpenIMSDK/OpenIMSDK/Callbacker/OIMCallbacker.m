...
- (void)onRecvGroupReadReceipt:(NSString* _Nullable)groupMsgReceiptList {
    OIMGroupMessageReceipt *receipts = [OIMGroupMessageReceipt mj_objectWithKeyValues:groupMsgReceiptList];
...
- (void)onConversationUserInputStatusChanged:(NSString *)changed {
    OIMInputStatusChangedData *info = [OIMInputStatusChangedData mj_objectWithKeyValues:changed];

    [self dispatchMainThread:^{
        if (self.onConversationUserInputStatusChanged) {
            self.onConversationUserInputStatusChanged(info);
        }
        
        [self.conversationListeners onConversationUserInputStatusChanged:info];
    }];
}
