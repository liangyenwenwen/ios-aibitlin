...
- (void)getGroupMessageReaderList:(NSString *)conversationID
                      clientMsgID:(NSString *)clientMsgID
    ...
    Open_im_sdkGetGroupMessageReaderList(callback, [self operationId], conversationID, clientMsgID, filter, offset, count);
}

- (void)sendGroupMessageReadReceipt:(NSString *)conversationID
                      clientMsgIDs:(NSArray< NSString *> *)clientMsgIDs
                        onSuccess:(OIMSuccessCallback)onSuccess
                          onFailure:(nullable OIMFailureCallback)onFailure {
    CallbackProxy *callback = [[CallbackProxy alloc]initWithOnSuccess:onSuccess onFailure:onFailure];
    
    Open_im_sdkSendGroupMessageReadReceipt(callback, [self operationId], conversationID, clientMsgIDs.mj_JSONString);
}
@end
