...
//            ProgressHUD.succeed("隐私协议")
        ...
        agreeStr.bs_set(textHighlightRange: agreementString.nsRange(from: range), color: .primaryColor, backgroundColor: nil) { [weak self]  containerView, text, range, rect in
            ...
            let language = String.getCurrentLanguage()
            if language.starts(with: "zh")  {
                SuperWebController.start((self?.navigationController!)!, uri: "https://protocol.dt-im.com/#/pages/registration/index?lang=zh")
            } else if language.starts(with: "th"){
                SuperWebController.start((self?.navigationController!)!, uri: "https://protocol.dt-im.com/#/pages/registration/index?lang=Thai")
            } else {
                SuperWebController.start((self?.navigationController!)!, uri: "https://protocol.dt-im.com/#/pages/registration/index?lang=en")
            }
        }
