...
class YFTransferAccountsView: UIView, ContainerCollectionViewCellDelegate  {
    ...
    private lazy var redPacketIcon: UIImageView = {
        let v = UIImageView(image: UIImage(named: "mine_transfer_accounts_log_icon"))
        return v
    }()
    ...
    private lazy var contentView: UIView = {
        ...
        v.backgroundColor = .init(hexString: "#FF9634")
        v.isUserInteractionEnabled = true
        return v
    }()
    
    private var controller: YFTransferAccountsController!
    
    func reloadData() {
        guard let controller else {
            return
        }
        titleLabel.text = (controller.currency ?? "") + String(format: " %.2f",controller.money ?? 0.00)
        if controller.transferAccountsStatus == 0{
            //未领取
            if controller.source.transferAccountsType == 0{
                //私聊转账
                if controller.instructions?.isEmpty == true{
                    statusLabel.text = "转账"
                }else{
                    statusLabel.text = "转账-" + controller.instructions!
                }
    ...
    func setup(with controller: YFTransferAccountsController) {
