...
public class ContactsViewController: UITableViewController {
    ...
    private func initView() {
        ...
        let findItem: UIBarButtonItem = {
            ...
            v.imageInsets = UIEdgeInsets(top: 0, left: 15, bottom: 0, right: 0)
...
//        }
    ...
    override open func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        if indexPath.section == SectionName.company.rawValue {
            #if ENABLE_ORGANIZATION
            let item = viewModel.companyDepartments.value[indexPath.row]
            let vc = DepartmentViewController(department: item.id, name: item.name!)
...
//            IMController.shared.getConversation(sessionType: .c2c, sourceId: user.ID!) { [weak self] (conversation: ConversationInfo?) in
