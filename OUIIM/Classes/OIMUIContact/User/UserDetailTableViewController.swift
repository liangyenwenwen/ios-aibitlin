...
//            ProgressHUD.success("ID已复制".innerLocalized())
    ...
    private func bindData() {        
        ...
        _viewModel.memberInfoRelay.subscribe(onNext: { [weak self] (memberInfo: GroupMemberInfo?) in
            ...
            if sself._viewModel.showSetAdmin == true, !sself.rowItems.contains(.setAdmin) {
                sself.rowItems.append(.spacer)
                sself.rowItems.append(.setAdmin)
            }
            
            if sself._viewModel.showMute == true, !sself.rowItems.contains(.mute) {
                sself.rowItems.append(.spacer)
                sself.rowItems.append(.mute)
            }
...
extension UserDetailTableViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if rowType == .profile || rowType == .moments {
            ...
            // 组织信息
            let cell = tableView.dequeueReusableCell(withIdentifier: MultipleTextLineCell.className, for: indexPath) as! MultipleTextLineCell
            cell.titleLabel.text = rowType.title
#if ENABLE_ORGANIZATION
            _viewModel.organizationInfo.value.forEach { info in
                cell.setText(title: "企业/组织".innerLocalized(), value: _viewModel.companyName)
                cell.setText(title: "部门".innerLocalized(), value: info.department?.name)
                cell.setText(title: "职位".innerLocalized(), value: info.member?.position)
            }
#endif
            return cell
        }
        ...
        } else if rowType == .setAdmin {
            cell.titleLabel.text = rowType.title
            cell.switcher.isHidden = false
            cell.switcher.isOn = info.roleLevel == .admin
            cell.switcher.rx.controlEvent(.valueChanged).subscribe(onNext: { [weak self] in
                self?._viewModel.toggleSetAdmin(toAdmin: cell.switcher.isOn, onSuccess: { r in
                    
                })
            }).disposed(by: cell.disposeBag)
        } else if rowType == .mute {
            cell.accessoryType = .disclosureIndicator
            cell.subtitleLabel.text = info.muteEndTime > NSDate().timeIntervalSince1970 * 1000 ?
            FormatUtil.getFormatDate(formatString: "yyyy-MM-dd HH:mm", of: Int(info.muteEndTime) / 1000) : nil
        }
    ...
    func tableView(_: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        case .mute:
            ...
            let vc = SetMuteViewController(userID: _viewModel.userId, groupID: groupID)
            navigationController?.pushViewController(vc, animated: true)
            
            vc.onSave = { [weak self] seconds in
                self?._viewModel.getUserOrMemberInfo()
            }
        default:
            break
        }
