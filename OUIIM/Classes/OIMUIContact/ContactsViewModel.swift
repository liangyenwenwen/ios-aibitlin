...
public class ContactsViewModel {
    ...
    func getFriendApplications() {
        IMController.shared.getFriendApplicationListAsRecipient { [weak self] (applications: [FriendApplication]) in
            ...
            // If the creation time of the obtained invitation is greater than the maximum browse time, it will be recorded and unread will be returned.
    ...
    func queryMyDepartmentInfo() {
        ...
        DispatchQueue.global(qos: .utility).async {
            ...
            group.notify(queue: .main) { [weak self] in
                self?.companyDepartments.accept(t)
            }
        }
#endif
    }
