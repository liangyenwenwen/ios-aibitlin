...
class FileRecordViewController: UIViewController {
    ...
    private func bindData() {
        ...
        _tableView.rx.modelSelected(MessageInfo.self).subscribe(onNext: { [weak self] ( message: MessageInfo) in
            guard let name = message.fileElem?.fileName, let size = message.fileElem?.fileSize, let url = message.fileElem?.sourceUrl else { return }
            let vc = FilePreviewViewController.init(messageID: message.clientMsgID, name: name, size: size, url: url)
