...
//            groupImage = cacheGroupImage(groupUnitImages, containerSize, avatarType, distance, avatarBgColor)
            groupImage = cacheGroupImageNew(groupUnitImages, containerSize, avatarType, distance, avatarBgColor)
...
//        var re = UIGraphicsImageRenderer.init(size: containerSize)
    ...
    private static func cacheGroupImageNew(_ maxSource: [UIImage], _ size: CGSize, _ avatarType: DCGroupAvatarType = .QQ, _ distance: CGFloat, _ avatarBgColor: UIColor) -> UIImage {
        var groupImage = UIImage()
        var itemAvaSize: CGSize = CGSize.zero
        var itemAvaPoint: CGPoint = CGPoint.zero
        let containerSize = CGSize(width: size.width, height: size.height)
        ...
        var render = UIGraphicsImageRenderer.init(size: containerSize)
        groupImage = render.image { rendererContext in
