...
public class IMController: NSObject {
    public static let addFriendPrefix = "com.kkkcdn.app/addFriend/"
    public static let joinGroupPrefix = "com.kkkcdn.app/joinGroup/"
...
//    }
    ...
    public func markMessageAsReaded(byConID: String, msgIDList: [String], onSuccess: @escaping CallBack.StringOptionalReturnVoid, onFailure: CallBack.ErrorOptionalReturnVoid? = nil) {
//        if msgIDList.isEmpty {
            Self.shared.imManager.markConversationMessage(asRead: byConID, onSuccess: onSuccess, onFailure: onFailure)
//        }
//        else {
//            Self.shared.imManager.markMessageAsRead(byMsgID: byConID, clientMsgIDs: msgIDList, onSuccess: onSuccess, onFailure: onFailure)
//        }
    }
    ...
    public func getGroupMessageReaderList(conversationID: String, clientMsgID: String, filter: Int = 0, offset: Int = 0, count: Int = 20, onSuccess: @escaping CallBack.GroupMembersReturnVoid) {
        Self.shared.imManager.getGroupMessageReaderList(conversationID, clientMsgID: clientMsgID, filter: filter, offset: offset, count: count) { ms in
            let r = ms?.compactMap({ $0.toGroupMemberInfo() }) ?? []
            onSuccess(r)
        } onFailure: { code, msg in
            print("\(#function) throw error:\(code), \(msg)")
        }
    ...
    public func sendGroupMessageReadReceipt(conversationID: String, clientMsgIDs: [String], onSuccess: @escaping CallBack.StringOptionalReturnVoid) {
        Self.shared.imManager.sendGroupMessageReadReceipt(conversationID, clientMsgIDs: clientMsgIDs, onSuccess: onSuccess) { code, msg in
            print("\(#function) throw error:\(code), \(msg)")
        }
    }
...
//            }
            ...
            if let handler = OIMApi.updateConversationCell {
                handler(clientMsgID, {res in
...
extension IMController: OIMAdvancedMsgListener {
    public func onRecvNewMessage(_ msg: OIMMessageInfo) {
        if msg.contentType.rawValue < 1000 || msg.contentType.rawValue == 1400,
    ...
    public func onRecvGroupRead(_ groupMsgReceiptList: OIMGroupMessageReceipt) {
        groupReadReceiptReceived.onNext(groupMsgReceiptList.toReceiptInfo())
    }
...
public class ConversationInfo: NSObject, Codable {
...
public class FullUserInfo {
    ...
    public var showName: String? {
        ...
        get {
            ...
            if let friendInfo {
                temp = friendInfo.remark?.isEmpty == false ? friendInfo.remark : friendInfo.nickname
            } else if let publicInfo {
                temp = publicInfo.nickname?.isEmpty == false ? publicInfo.nickname : publicInfo.userID
            }
...
extension OIMGroupMessageReceipt {
    func toReceiptInfo() -> GroupMessageReceipt {
        let item = GroupMessageReceipt()
        item.conversationID = conversationID
        item.groupMessageReadInfo = groupMessageReadInfo.compactMap({ $0.toReceiptInfo() })
        
        return item
    }
}

extension OIMGroupMessageReadInfo {
    func toReceiptInfo() -> GroupMessageReadInfo {
        let item = GroupMessageReadInfo()
        item.clientMsgID = clientMsgID
        item.hasReadCount = hasReadCount
        item.unreadCount = unreadCount
        item.readMembers = readMembers.compactMap({ $0.toGroupMemberInfo() })
...
//
///// 获取用户信息
//public class QueryUserInfo: UpdateUserInfoRequest {}
//
