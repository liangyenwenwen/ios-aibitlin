...
class FaceManageViewController: UIViewController, UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if indexPath.item == 0 {
            ...
            // Set the closure for the delete button tap event
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if indexPath.item == 0 {
            // Add Image cell tapped
            // Implement your logic to add an image (e.g., show image picker)
            // For simplicity, let's assume you have an imageURL for the new image
    ...
    // MARK: Handle Long Press Gesture for Delete Action
    
    @objc private func handleLongPress(_ gesture: UILongPressGestureRecognizer) {
        if gesture.state == .began {
            let touchPoint = gesture.location(in: collectionView)
            if let indexPath = collectionView.indexPathForItem(at: touchPoint) {
                // Handle long press on an image cell (delete action)
...
extension FaceManageViewController {
    ...
    @objc private func handleAddImageButtonTapped() {
        // Handle add image button tap event
        // Implement your logic to select a photo (e.g., show image picker)
