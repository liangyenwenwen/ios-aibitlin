...
class CommentInputView: UIView {
    fileprivate(set) lazy var contentView: UIView = {
        let v = UIView()
        v.frame = CGRect(x: 0, y: kScreenHeight, width: kScreenWidth, height: contentMinHeight)
    ...
    fileprivate(set) lazy var textView: MomentTextView = {
        let v = MomentTextView()
        v.placeholder = "评论".innerLocalized()
        v.lineBreak = false
        ...
        v.frame = CGRect(x: 15, y: surplusHeight / 2, width: kScreenWidth - 30, height: contentMinHeight - surplusHeight)
        return v
    }()
    ...
    func scrollForComment(_ rect: CGRect) {
        if keyboardHeight > 0 {
            ...
            NotificationCenter.default.post(name: NSNotification.Name.list.contentOffset, object: 0)
        }
    }
}
...
fileprivate extension CommentInputView {
    ...
    func updateTop(rect: CGRect, duration: Double) {
        ...
        var top: CGFloat = 0
        if keyboardH > 0 {
            top = kScreenHeight - contentView.frame.height - keyboardHeight
        } else {
            top = kScreenHeight
        }
        if ctTop == top { return }
    ...
    func updateHeight(_ height: CGFloat) {
        ...
        ctTop = kScreenHeight - ctHeight - keyboardHeight
